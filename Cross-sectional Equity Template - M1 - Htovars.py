...
class CumMomentum(CustomFactor):
    # Default inputs
    inputs = [USEquityPricing.close]
    ...
    def compute(self, today, assets, out, close):
        cumMomentum = 0;
        for i in range(11):
            cumMomentum = cumMomentum + (close[i - 10] - close[i - 9]) / abs(close[i - 9])
        out[:] = cumMomentum
...
def make_pipeline():
    ...
    CumulativeMomentum = CumMomentum(window_length=11)
    ...
    combined_factor = (
            CumulativeMomentum_winzorized.zscore() -
            std_dev_winsorized.zscore() +
            ten_day_momentum_winsorized.zscore() +
